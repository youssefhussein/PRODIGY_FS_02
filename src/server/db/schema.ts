...
export const employee = sqliteTable(
	...
	(d) => ({
		...
		position: d.text({ length: 256 }),
		department: d.text({ length: 256 }),
		salary: d.integer({ mode: "number" }),
		hireDate: d.integer({ mode: "timestamp" }),
		createdAt: d
			.integer({ mode: "timestamp" })
			.default(sql`(unixepoch())`)
			.notNull(),
	...
	(t) => [index("employee_user_id_idx").on(t.userId)],
);
